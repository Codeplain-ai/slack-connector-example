...
Accessing Slack API methods requires an OAuth token — read more about installing with OAuth.

Each of these API methods is fully documented on our developer site at docs.slack.dev.
Sending a message
...
Your app's bot user needs to be in the channel (otherwise, you will get either not_in_channel or channel_not_found error code). If your app has the chat:write.public scope, your app can post messages without joining a channel as long as the channel is public. See the chat.postMessage API method for more info.
...
See the chat.postEphemeral API method for more details.
...
The Python Slack SDK provides a chat_stream() helper utility to streamline calling these methods.

See the Streaming messages section of the Bolt for Python docs for implementation instructions.
...
The method also provides you an opportunity to request user feedback on your app's responses using the feedback buttons block element within the context actions block. The user will be presented with thumbs up and thumbs down buttons which send an action to your app when pressed.
...
See Formatting messages with Block Kit below for more details on using Block Kit with messages.
Formatting messages with Block Kit

Messages posted from apps can contain more than just text; they can also include full user interfaces composed of blocks using Block Kit.
...
You can use Block Kit Builder to prototype your message's look and feel.
Threading messages
...
Refer to the threading messages page for more information.
Updating a message
...
See the chat.update API method for formatting options and some special considerations when calling this with a bot user.
Deleting a message
...
See the chat.delete API method for more details.
Conversations
...
Refer to using the Conversations API for more information.
Direct messages
...
See the conversations.open API method for additional details.
Creating channels
...
See the conversations.create API method for additional details.
...
See the conversations.info API method for more details.
...
See the conversations.list API method for more details.
...
See the conversations.members API method for more details.
...
See the conversations.join API method for more details.
...
See the conversations.leave API method for more details.
Opening a modal
...
See the views.open API method more details and additional parameters.

Also, to run the above example, the following Slack app configurations are required.

To run the above example, the following app configurations are required:
...
In response to view_submission requests, you can tell Slack to update the current modal view by having "response_action": update and an updated view. There are also other response_action types, such as errors and push. Refer to the modals page for more details.
...
See the views.update API method for more details.

If you want to push a new view onto the modal instead of updating an existing view, see the views.push API method.
Emoji reactions
...
See the reactions.add and reactions.remove API methods for more details.
Uploading files
...
See the files.upload API method for more details.
...
See the files.remote.add API method for more details.
Calling API methods
...
try:
    ...
    for user in users:
        print(f"{user['id']} - {user['name']} - {user.get('real_name', '')}")
...
The webhook client is available in asynchronous programming using the standard asyncio library. You use AsyncWebhookClient instead. AsyncWebhookClient internally relies on the AIOHTTP library, but it is an optional dependency. To use this class, run pip install aiohttp beforehand.
...
For asyncio apps, Async prefixed corresponding modules are available. All the methods in those methods are async/await compatible. Check the source code for more details.
Rate limits

When posting messages to a channel, Slack allows apps to send no more than one message per channel per second. We allow bursts over that limit for short periods; however, if your app continues to exceed the limit over a longer period of time, it will be rate limited. Different API methods have other limits — be sure to check the rate limits and test that your app has a graceful fallback if it should hit those limits.
...
Since v3.9.0, the built-in RateLimitErrorRetryHandler is available as an easier way to do retries for rate limited errors. Refer to the RetryHandler section for more details.

Refer to the rate limits page for more information.
